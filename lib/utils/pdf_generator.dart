...
class PdfGenerator {
  ...
  /// Generates PDF and shares for a report (no month/year, uses date range)
  static Future<void> generateAndShareReportPdf(List<WorkEntry> entries) async {
    final dateRange = ReportService.getDateRangeString(entries);
    final pdfBytes = await _generateReportPdfBytes(entries, dateRange);
    final fileName = 'arbetsrapport_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  /// Generates PDF and sends via email for a report
  static Future<void> generateAndSendReportEmail(List<WorkEntry> entries) async {
    final dateRange = ReportService.getDateRangeString(entries);
    final pdfBytes = await _generateReportPdfBytes(entries, dateRange);
    final fileName = 'arbetsrapport_${DateTime.now().millisecondsSinceEpoch}.pdf';
    ...
    final subject = 'Arbetsrapport $dateRange';
    final body = '''Hej!

Här kommer arbetsrapporten för $dateRange.
...
Med vänliga hälsningar''';
  ...
  /// Internal function to generate PDF bytes for a report
  static Future<List<int>> _generateReportPdfBytes(
